				...
				<div className="collapse navbar-collapse" id="navbarTogglerDemo02">
					<ul className="navbar-nav mb-lg-0">
						...
						/* hidden={store.buttons_admin_tech.login} */>
							<Link to="/Favorites" className="nav-link text-decoration-none position-relative" id="dropdownMenuButton1" data-bs-auto-close="outside" data-bs-toggle="dropdown" aria-expanded="false">
								<span className="nav-span">Favorites</span>
								<span className="position-absolute top-25 ms-2 start-100 translate-middle badge rounded-pill bg-danger" /* hidden={store.favorites.length == 0 ? true : false} */>
					...
					<ul className="navbar-nav mb-lg-0 ms-auto">
						<li className="nav-item ms-2" /* hidden={store.buttons_admin_tech.login}*/>
							<Link to="/login" className="text-decoration-none" >
								<button className="btn btn-warning"><i className="fa-solid fa-user-astronaut me-2"></i>LogIn</button>
							</Link>
						</li>
						<li className="nav-item ms-2" /* hidden={store.buttons_admin_tech.account} */>
							<Link to="/" className="text-decoration-none" >
								<button className="btn btn-warning" onClick={() => actions.logout()}><i class="fa-solid fa-rocket me-2"></i>LogOut</button>
							</Link>
						</li>
					</ul>
				</div>
