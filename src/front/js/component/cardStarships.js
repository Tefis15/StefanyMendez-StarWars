...
import { StarshipsDetailsModal } from "./starshipsDetailsModal";


export const CardStarships = ({ starship }, ...props) => {
        ...
        <>
                <div className="card card-all">
                    ...
                    <button className="btn btn-yellow fs-5 position-absolute end-0 me-2 mt-2"
                        onClick={() => {
                            !!store.isLoggedIn ?
                                store.favoritesStarships.find((favorite) => favorite.starships_uid.uid === starship.uid) ?
                                    actions.deleteFavoritesStarships(starship.uid)
                                    :
                                    actions.addFavoritesStarships(starship.uid)
                                : Swal.fire({
                                    icon: 'warning',
                        ...
                        <i className={store.favoritesStarships.find((favorite) => favorite.starships_uid.uid === starship.uid) ?
                            "fa-solid fa-heart"
                            : "fa-regular fa-heart"}>
                        </i>
                    </button>
                    <div className="card-body text-center">
                        <h4 className="card-title text-white text-start">{starship.name}</h4>
                        <button className="btn btn-yellow mt-4 me-2 fw-bold">
                            <Link className="link text-decoration-none text-dark"
                                to={`/starships/details/${starship.uid}`}
                                onClick={() => actions.getStarshipsById(starship.uid)}>
                                Details
                            </Link>
                        </button>
                        ...
                        <button className="btn btn-yellow mt-4 me-2 fw-bold" hidden={store.userLogin.role == "admin" ? false : true}
                            onClick={() => {
                                actions.getStarshipsById(starship.uid)
                        ...
                        <button className="btn btn-trash mt-4 fw-bold" hidden={store.userLogin.role == "admin" ? false : true}
                            ...
                            `,
                                    ...
                                    if (result.isConfirmed) {
                                        actions.deleteStarships(starship.uid, starship.name)
                                    }
                                })
                            }>
            ...
            <StarshipsDetailsModal show={store.showModalDetails} />
        </>
    )
}
